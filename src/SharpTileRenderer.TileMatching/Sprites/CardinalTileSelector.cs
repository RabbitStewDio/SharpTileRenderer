...
namespace SharpTileRenderer.TileMatching.Sprites
{
    /// <summary>
    /// <para>
    ///   Matches a tile with possible neighbouring tiles along the cardinal
    ///   direction.  This produces 16 different match combinations to choose the
    ///   right tile. 
    ///  </para>
    /// <para>
    ///   The selector uses two different matchers. The self-matcher is a strict
    ///   match  selecting the tile type to be rendered. The neighbour-matchers
    ///   are used to decide whether this strictly matched tile connects with any
    ///   of the neigbouring tiles.
    /// </para>
    /// <para>
    ///   Example: When matching rivers, the a river tile should only be rendered
    ///   when the map tile in question is a river. However, rivers flow into the
    ///   ocean, so the river represented by this tile should treat oceans as
    ///   connectable tiles, in the same way a neighbouring river tile would be
    ///   connected.
    /// </para>
    /// </summary>
    /// <typeparam name="TRenderTile">The target tile type as provided by the tile
    /// registry.</typeparam>
