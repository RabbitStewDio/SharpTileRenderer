...
using SharpTileRenderer.TexturePack.Atlas;
...
{
    public class XnaTextureOperations : ITextureOperations<XnaTexture, Color>
    {
        ...
        public XnaTextureOperations(GraphicsDeviceManager deviceManager)
        {
        ...
        public TextureCoordinateRect ToNative(IntDimension context, TextureCoordinateRect src)
        {
            return src;
        }

        public TextureCoordinateRect ToNormalized(IntDimension context, TextureCoordinateRect src)
        {
            return src;
        }
                                                     ...
                                                     BoundedTextureData<Color> mask)
        {
            ...
            for (var y = 0; y < height; y += 1)
            {
                for (var x = 0; x < width; x += 1)
                {
                    ...
                    var textureIndex = y * width + x;
                    retval[textureIndex] = c * (a.A / 255f);
                }
            }
                                                      ...
                                                      BoundedTextureData<Color> foreground)
        {
            ...
            for (var y = 0; y < height; y += 1)
            {
                for (var x = 0; x < width; x += 1)
                {
                    ...
                    var tgtW = tgt * tgt.A;
                    retval[textureIndex] = new Color(srcW.R + tgtW.R, srcW.G + tgtW.G, srcW.B + tgtW.B, srcW.A + tgtW.A);
                }
            }
        ...
        public TextureCoordinateRect TileAreaForCardinalDirection(IntDimension ts, TextureQuadrantIndex dir)
        {
            var w = ts.Width;
            var h = ts.Height;
        ...
        {
            return new XnaTexture(name, texture.Texture, clipRegion.Clip(texture.Bounds));
        }

        public XnaTexture CreateTexture(string name, IntDimension textureSize, bool clearToTransparentBlack = true)
        {
            var texture = new Texture2D(deviceManager.GraphicsDevice, textureSize.Width, textureSize.Height, false, SurfaceFormat.Color);
            ...
            return new XnaTexture(name, texture);
        }
